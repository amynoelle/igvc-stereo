...
void log2file( const std::string &text )
{
    std::ofstream log_file(
        "log_file.csv", std::ios_base::out | std::ios_base::app );
    log_file << text;
}
...
int main(int argc, char** argv) {
    ...
    */
    ...
    if (isLogging) log2file("top,afgrab,afRmeasure,afRImage,afFilter,afDwnImg,b4Forloop,afForloop,b4Sleep, CLOCKS_PER_SEC\n"); 
    ...
    while (nh.ok()) {
        ...
        if (!zed->grab(dm_type)) { // grabs a new image
	        ...
	        if (isLogging) outStream << clock() << ","; // TIMER: afRImage 2.6ms/6.1% of time
		
		    //Filter the image for white lines
		    cv::gpu::GpuMat cv_filteredImage = color_filter.findLines(host_image);
	        if (isLogging) outStream << clock() << ","; // TIMER: afFilter 21.1/9.1% of time
		    cv_filteredImage.download(cloud_image); //move image to cpu memory
	        if (isLogging) outStream << clock() << ","; // TIMER: afDwnImg 0.65ms/1.6% of time
		...
		free(cpu_cloud);
	        ...
	        if (isLogging) outStream << clock() << ","<< CLOCKS_PER_SEC << "\n"; // TIMER: b4Sleep 0.29ms//0.7% of time	
